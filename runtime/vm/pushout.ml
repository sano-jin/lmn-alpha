...
let push_atom functor_ new_atom atom_lists =
  ...
  | None ->
      ...
      let new_atom_elt =
        (* atom_list に破壊的に挿入を行う *)
        DList.insert_first atom_list new_atom
      in
      (new_atom_elt, AtomLists.add functor_ atom_list atom_lists)
  | Some atom_list -> (DList.insert_first atom_list new_atom, atom_lists)

(** アトムリストからアトムを削除する *)
let remove_atom functor_ atom_elem atom_lists =
  (* このアトムはアトムリストに必ず存在するはずなので，find_opt でなく，find で必ず取れるはず *)
  let atom_list = AtomLists.find functor_ atom_lists in
  DList.remove atom_list atom_elem;
  if DList.is_empty atom_list then AtomLists.remove functor_ atom_lists
  else atom_lists
...
let pushout register env atom_lists = function
  | PushAtom (reg_i, functor_) ->
      ...
      *)
      ...
      let new_atom_elt, atom_lists = push_atom functor_ new_atom atom_lists in
  ...
  | ReLink (reg_i, port_i, reg_j, port_j) ->
      ...
      *)
      let ys, port_j, atom_ref_j = deref_lhs_register register reg_j port_j in
  ...
  | Connect (reg_i, port_i, reg_j, port_j) ->
      ...
      *)
      let xs, port_i, atom_ref_i = deref_lhs_register register reg_i port_i in
  ...
  | SetData (reg_i, port_i, var) ->
      ...
      let atom_ref = register.(reg_i) in
      let _, xs = DList.value atom_ref in

      xs.(port_i) <- List.assoc var env;
      atom_lists
