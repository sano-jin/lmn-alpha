...
and link =
  | NormalLink of int * vm_atom_elt  (** (port_i, atom) *)
  | VMIntData of int
...
(** 初期状態のレジスタを確保する *)
let init_register size : registers = Array.make size null_atom_elt

(** NormalLink コンストラクタを剥がす *)
let retrieve_normal_link = function
  | NormalLink (port_i, atom_ref) -> (port_i, atom_ref)
  | _ -> failwith @@ "Bug: cannot retrieve NormalLink constructor"
 ...
 *)
let free_atom atom_ref = DList.map_value (first @@ ( ^ ) "~") atom_ref

(** アトムリストの Map．
    - それぞれのファンクタ毎のアトムリストの Map になっている．
*)
module AtomLists = Map.Make (struct
  type t = string * int
...
(** アトムリストの dumper *)
type vm_dump_link = DLink of int * int | DIntData of int

(** リンクの解消 *)
let dump_link atom2atom_i = function
  | NormalLink (port_i, atom_elt_ref) ->
      DLink (port_i, List.assq (DList.value atom_elt_ref) atom2atom_i)
  | VMIntData i -> DIntData i
...
let dump_atom_list atom_lists =
  let atoms =
    List.concat_map (DList.to_list <. snd) @@ AtomLists.bindings atom_lists
  in
