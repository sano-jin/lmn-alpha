...
let compare_functor (_, (p, xs)) (_, (q, ys)) =
  compare
    (atom_name_type_of p, List.length xs)
    (atom_name_type_of q, List.length ys)
...
(** アトムリストを木構造へ変換する
    - あまり美しくないのでリファクタが必要
*)
let rec tree_of_link src_atom_i src_port_i (free_link, link2link_name, atoms) =
  function
  | DIntData i -> ((free_link, link2link_name, atoms), IntData i)
  | DLink (dst_port_i, dst_atom_i) -> (
      let dst_link = (dst_port_i, dst_atom_i) in
      let leaf_of_link dst_link =
        match List.assoc_opt dst_link link2link_name with
        | None ->
            let link = Link ("L" ^ string_of_int free_link) in
            ( ( succ free_link,
                ((src_port_i, src_atom_i), link) :: link2link_name,
                atoms ),
              link )
      ...
      | Some (p, xs) ->
          if List.length xs - 1 = dst_port_i then
            (* 最終リンクで接続されているなら埋め込みが可能 *)
            tree_of_atom
              (free_link, link2link_name, atoms)
              (dst_atom_i, (p, dropLast1 xs))
...
and tree_of_atom (free_link, link2link_name, atoms) (src_atom_i, (p, xs)) =
  ...
  let env, xs =
    fold_left_mapi (tree_of_link src_atom_i)
      (free_link, link2link_name, atoms)
      xs
  in
  (env, Atom (p, xs))
...
let pretty_print atoms =
  ...
  @@ forest_of_atoms @@ tpl_sort @@ functor_sort @@ Util.shuffle atoms
