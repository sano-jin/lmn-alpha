 ...
 *) 
let rec run_many tracer rules i atom_list =
  tracer i atom_list; 
  match Eval.run_once atom_list rules with
  | None -> atom_list
  | Some (_, atom_list) -> run_many tracer rules (succ i) atom_list


(** 初期状態を構築した後，最後まで実行して，最終状態を表示する *)
let run tracer dumper (init_insts, rules) =
  let initial_atom_list = init_atoms init_insts in
  let final_state = run_many tracer rules 0 initial_atom_list in
...
let main () =
  ...
  if prop.compile_only then
    ...
    let dumper = (if prop.verbose then Dump.string_of_atom_list else Pretty.pretty_print) <. dump_atom_list in
    let tracer = if prop.trace
		 then fun i atoms -> print_endline @@ string_of_int i ^ ": " ^ dumper atoms
		 else fun _ _ -> ()
    in
    run tracer dumper @@ insts
