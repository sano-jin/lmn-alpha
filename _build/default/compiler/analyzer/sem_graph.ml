...
let string_of_local_port_binding (key, ports) =
  string_of_int key ^ " -> {" ^ String.concat ", " (List.map string_of_port ports) ^ "}"


(** local_port_map の dumper *)
let string_of_local_port_map =
  String.concat ", " <. List.map string_of_local_port_binding <. LocalPortMap.bindings 
...
type a_rule = ARule of string * (a_graph * (a_graph * c_conn list))
...
let string_of_graph (ports, atoms) =
 "{" ^ string_of_local_port_map ports ^ "} " ^ string_of_atoms (List.map snd atoms)


let string_of_rule = function
  | ARule (rule_name, (p, (q, rhs_conns))) ->
     rule_name ^ " @@ " ^ string_of_graph p ^ " :- " ^ string_of_graph q ^ ", " ^ string_of_connectors rhs_conns
