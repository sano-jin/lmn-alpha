...
let corelang_of_rule rule_name
    ((lhs_free_links, (lhs_atoms, lhs_conns)), lhs_rules) guard
    ((rhs_free_links, rhs_graph), rhs_rules) =
  if lhs_rules <> [] then raise @@ CompileError "Rule(s) cannot appear on LHS"
  else if rhs_rules <> [] then
    raise @@ CompileError "Rule(s) on RHS is/are not supported"
  else if lhs_free_links <> rhs_free_links then
    let free_links = sym_diff lhs_free_links rhs_free_links in
    raise
    @@ CompileError
         ("Free link(s) " ^ string_of_link_set free_links ^ " of a rule")
  else if lhs_conns <> [] then
    raise
    @@ CompileError
         ("Unabsorbable connector(s) "
         ^ string_of_connectors lhs_conns
         ^ " on LHS of a rule")
  else
    let guards, guard_rules = Partition.partition_proc guard in
    if guard_rules <> [] then
      raise @@ CompileError "Rule(s) cannot appear on guard"
    else CRule (rule_name, (lhs_atoms, guards, rhs_graph))
