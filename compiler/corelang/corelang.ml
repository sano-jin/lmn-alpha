...
and corelang_of_rule (rule_name, (p, q)) =
  Rule.corelang_of_rule rule_name (corelang_of_proc p) (corelang_of_proc q)
...
let corelang_of_ast ast =
  ...
  if not @@ LinkSet.is_empty frees then
    raise @@ CompileError ("Free link(s) of a program " ^ string_of_link_set frees ^ " is not supported")
  else atoms, rules
