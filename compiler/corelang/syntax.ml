...
type c_rule =
  | CRule of
      string * (c_atom list * Parse.arg list * (c_atom list * c_conn list))
...
let string_of_rhs (atoms, conns) =
  ...
  @ List.map string_of_connector conns

let string_of_rule = function
  | CRule (rule_name, (p, guard, q)) ->
      rule_name ^ " @@ " ^ string_of_atoms p ^ " :- "
