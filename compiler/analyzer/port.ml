...
let add_local_port atom_i port_i =
  flip @@ function
  | LocalLink x ->
      LocalPortMap.update x
        (Option.some <. List.cons (atom_i, port_i) <. maybe [])
  | FreeLink _ -> id (* 自由リンクなら何もしない *)
  | CProcCtx _ -> id (* プロセス文脈も何もしない *)
  | CIntData _ -> id
(* データアトムも何もしない *)
...
let local_ports_of_atoms atoms =
  List.fold_left
    (uncurry <. flip (fold_lefti <. add_local_port))
    LocalPortMap.empty
