...
(** ルールの解析を行う *)
let sem_graph_of_rule
    (CRule (rule_name, (lhs_atoms, guard, (rhs_atoms, rhs_conns)))) =
  ARule
    ( rule_name,
      ( sem_graph_of_atoms lhs_atoms,
        guard,
...
let sem_graph_of_process (atoms, rules) =
  (sem_graph_of_atoms atoms, List.map sem_graph_of_rule rules)
