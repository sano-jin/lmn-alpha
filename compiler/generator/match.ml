...
let check_arg src_reg_i (local_port_map, reg_tbl) src_port_i = function
  | CIntData _ -> failwith @@ "Error: data atom on LHS is not supported yet"
  | CProcCtx var ->
      ((local_port_map, reg_tbl), [ BindData (var, src_reg_i, src_port_i) ])
  | FreeLink x ->
      (* 自由リンクはマッチングに関しては特に何も行わない．後でルール右辺できちんと接続するための情報を取得しておくだけ *)
      ( ( local_port_map,
          {
            reg_tbl with
            free2reg_i = (x, (src_reg_i, src_port_i)) :: reg_tbl.free2reg_i;
          } ),
        [] )
  | LocalLink x -> (
      let dst_reg_i, reg_tbl = get_free_reg_i reg_tbl in
      (* リンクの Dereference を行うためのレジスタ番号を確保する  *)
      ...
      @@
      if List.mem x reg_tbl.matched_local_links then (reg_tbl, [])
        (* すでにマッチングが完了した局所リンクの場合は何もしない *)
        (* lmntal の局所リンクは常に相互につながっているため，相手がたをもう一度確認する必要はないため *)
      else
        let reg_tbl =
          {
            reg_tbl with
            matched_local_links = x :: reg_tbl.matched_local_links;
          }
        in
        let deref_atom =
          DerefAtom (dst_reg_i, src_reg_i, src_port_i, dst_port_i)
        in
        (* 局所リンクの場合は [DerefAtom] 命令を用いる *)
        match List.assoc_opt dst_atom_i reg_tbl.atom2reg_i with
        | None ->
            ( {
                reg_tbl with
                atom2reg_i = (dst_atom_i, dst_reg_i) :: reg_tbl.atom2reg_i;
              },
              [ deref_atom ] )
...
let check_atom (local_port_map, reg_tbl) reg_i (p, xs) =
  ...
  let check_ref_neqs =
    let matched_atoms =
      List.filter (( = ) functor_ <. fst) reg_tbl.matched_functors
    in
  ...
  let reg_tbl =
    {
      reg_tbl with
      matched_functors = (functor_, reg_i) :: reg_tbl.matched_functors;
    }
  in
  let xs = List.mapi pair xs in
  let env, check_args =
    List.fold_left_map (uncurry <. check_arg reg_i) (local_port_map, reg_tbl) xs
  in
...
let find_atom ((local_port_map, reg_tbl) as env) (atom_i, atom) =
  ...
  | None ->
      (* Could not dereference. has not matched yet. *)
      (* アトムリストの先頭から随時アトムへの参照をレジスタ reg_i に格納する命令を発行する *)
      let reg_i, reg_tbl = get_free_reg_i reg_tbl in
      let reg_tbl =
        { reg_tbl with atom2reg_i = (atom_i, reg_i) :: reg_tbl.atom2reg_i }
      in
