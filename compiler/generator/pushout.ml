...
*)
let set_link_of_link reg_tbl atom2reg_i src_reg_i src_port_i (local_port_map: local_port_map) = function
  | FreeLink x ->
  ...
  | LocalLink x -> 
	...
	hyper へ拡張するなら，リストの最終要素以外のリストという風にする必要がある *)
     ...
     let local_port_map = LocalPortMap.update x (Option.map roll) local_port_map in
...
*)
let set_links_of_atom reg_tbl atom2reg_i (local_port_map: local_port_map) (atom_i, (_, xs)) =
  let src_reg_i = List.assoc atom_i atom2reg_i in 
  (fold_left_mapi (set_link_of_link reg_tbl atom2reg_i src_reg_i) local_port_map xs: local_port_map * rhs_inst list)

			   
(* link2reg_i は（恐らくは）単相性制限のために eta 変換できなかった *)  
let set_links_of_atoms reg_tbl atom2reg_i local_port_map atoms =
  second List.concat
  @@ List.fold_left_map (set_links_of_atom reg_tbl atom2reg_i) local_port_map atoms
...
let push_atoms reg_tbl ((local_port_map, atoms), connectors) =
  ...
  let _, set_or_re_links  = set_links_of_atoms reg_tbl atom2reg_i local_port_map atoms in
