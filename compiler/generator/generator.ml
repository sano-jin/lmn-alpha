 ...
 *)
let gen_ic_of_rule (ARule (rule_name, (lhs, guard, rhs))) =
  let reg_tbl, lhs_insts = Match.match_ lhs in

  let reg_size, rhs_insts = Pushout.push_atoms reg_tbl rhs in

  (* debug_print "genereted rhs_insts" @@ string_of_rhs_insts rhs_insts;  *)
  (* debug_print "register_size" @@ string_of_int reg_size; *)
  (rule_name, (reg_size, (lhs_insts, guard, rhs_insts)))
